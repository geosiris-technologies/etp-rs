...
[[XXX]] : optional parameter
        ...
        */
        ...
        if command_and_params.len() > 1 {
            ...
            let getress = GetDataObjects{
                uris: uris_params.iter().enumerate()
    ...
    */
        ...
        let get_dataspaces = GetDataspaces{store_last_write_filter:None};
                ...
                dataspaces: HashMap::from([
                    ("0".to_string(),
                        Dataspace::default_with_params(
                            if dataspace_name.contains("eml:///") {dataspace_name.to_string()}
                            else {format!("eml:///dataspace('{dataspace_name}')")},
                            date_now(),
                            date_now(),
                        )
                    )])
    ...
    */
        ...
        let close = CloseSession{reason: "Bye Bye".to_string()};
