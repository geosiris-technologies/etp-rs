...
// cargo run -- -u ws://rdms.geosiris.com/etp/ --login admin --password openSesame


async fn test_tokio(args: Args){
    //let _connet_address = "wss://stream.bybit.com/realtime";
    //let url = url::Url::parse(connet_address).unwrap();


    let (tx, rx) = channel::mpsc::unbounded::<tungstenite::protocol::Message>();
    // let _heartbeat_handles = tokio::spawn(send_message(tx));
    ...
    send_message(&tx, &mut etp_connection, get_request_session().as_protocol_message()).await;
...
async fn send_message(tx: &channel::mpsc::UnboundedSender<Message>, etp_connection: &mut EtpConnection, msg: ProtocolMessage) {
...
async fn receive(mut read:  SplitStream<WebSocketStream<tokio_tungstenite::MaybeTlsStream<tokio::net::TcpStream>>>, args: Args){
            ...
            Ok(msg) => {
                match msg{
                    tungstenite::Message::Binary(b) => {
                        ...
                        let decoded_msg = std::panic::catch_unwind(
                            || decode_message(&b)
                        );
                        if let Ok(decoded) = decoded_msg{
                            print_protocol_message(decoded.1.unwrap(), &args); 
                        }else if let Err(not_decoded) = decoded_msg{
                            log(&format!("ERR) Failed to decode message: {:?}\n{:?}", not_decoded, b), &args.log_file);
                        }
                    }
                    tungstenite::Message::Text(b) => { log(&format!(" TEXT {:?}", b), &args.log_file); }
...
async fn interface(tx: &channel::mpsc::UnboundedSender<Message>, etp_connection: &mut EtpConnection){
    loop {
...
#[derive(Parser, Debug)]
#[clap(author="Valentin Gauthier", version, about="A simple etpclient that use etpproto")]
struct Args {
    #[arg(short, long)]
    url: String,
...
fn print_protocol_message(msg: ProtocolMessage, args: &Args){
    match msg{
        Discovery_GetResourcesResponse(mut grr) => {
            log(&format!("GetRangesResponse : {} entity found", grr.resources.len()), &args.log_file);
        ...
        Dataspace_GetDataspacesResponse(dsr) => {
            log(&format!("Dataspaces : {:?}", dsr.dataspaces.iter().map(|d| &d.uri).collect::<Vec<&String>>()), &args.log_file);
        },
...
fn format_resource(r: &Resource) -> String{
    format!("{:?}: {:?}\n\tsources : {:?}\n\ttargets: {:?}", r.name, r.uri, r.source_count, r.target_count)
}

fn format_data_object(d_o: &DataObject) -> String{
    format!("{:?}: {:?}\n\tContent : {}\n", d_o.resource.uri, d_o.format, String::from_utf8(d_o.data.to_vec()).unwrap())
}